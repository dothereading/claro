"""v10 cardinal GRPO reward for the A2 simplifier.
...
Each component lives in its own module and returns a value in [0, 1]:
  * level_band  (reward.level_band)  — deterministic readability-band match
  * vocab       (reward.vocab)       — deterministic hard-word penalty
  * fidelity    (reward.fidelity)    — one LLM judge call per rollout
  * gates       (reward.gates)       — hard 0/1 format mask (lifted from v9)
