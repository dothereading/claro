...
*   [x] **Reward sanity tools**: `python rewards.py audit <jsonl>` scores any dataset with all rewards; `python rewards.py variety --adapter <path>` samples G rollouts per prompt and reports per-group reward std (the GRPO advantage signal); `compute_variety()` is also exposed as a library helper.
